...
from io_classes.file_manager import FileManager
...
class TaskData:
    ...
    def load_task(self, encoded_name: str) -> None:
        ...
        if encoded_name not in self.tasks:
            if self.file_manager.json_file_exists(encoded_name):
                task = self.file_manager.load_json_file(encoded_name)
                try:
    ...
    def save_task(self, encoded_name: str) -> None:
        ...
        # save task
        if encoded_name in self.tasks:
            self.file_manager.save_json_file(encoded_name, self.tasks[encoded_name])
        else:
            raise ValueError(f"There is no task {encoded_name}")
    ...
    def exists_task(self, encoded_task: str) -> bool:
        """
